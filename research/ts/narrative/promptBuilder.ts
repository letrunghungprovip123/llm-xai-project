...
export function buildPrompt(
  ...
  const systemMessage = buildSystemMessage(packageItem);
...
function buildSystemMessage(packageItem: EvidencePackage): string {
  const semantics = resolveTargetSemantics(packageItem);
  if (!semantics) {
    // Frozen legacy text: historical Home Credit prompt hashes remain reproducible
    // when the evidence package does not opt into multi-dataset semantics.
    return [
  ...
  return [
    "Bạn là hệ thống sinh lời giải thích dự đoán rủi ro tín dụng bằng tiếng Việt.",
    "Mục tiêu quan trọng nhất là trung thực với bằng chứng được cung cấp.",
    "Chỉ sử dụng dữ liệu nằm trong prompt. Không tự bổ sung thông tin về đối tượng được dự đoán.",
    "Không diễn đạt SHAP như quan hệ nhân quả ngoài đời thực.",
    `Endpoint dự đoán của thí nghiệm là: ${semantics.prediction_subject}.`,
    "Không khẳng định chắc chắn kết quả thực tế sẽ xảy ra hoặc không xảy ra; đây chỉ là dự đoán của mô hình.",
    "Không đưa lời khuyên tài chính cá nhân hoặc quyết định tín dụng.",
    "Phải trả về đúng một JSON object hợp lệ, không Markdown, không code fence, không văn bản bên ngoài JSON.",
  ].join("\n");
}
...
function buildUserMessage(packageItem: EvidencePackage): string {
  ...
  const targetSemantics = resolveTargetSemantics(packageItem);
  if (targetSemantics) {
    sections.push(buildTargetSemanticsSection(targetSemantics));
  }
...
function buildTargetSemanticsSection(
  ...
  return [
    ...
    JSON.stringify(
      {
        semantic_name: semantics.semantic_name,
        prediction_horizon: semantics.prediction_horizon ?? null,
        prediction_subject: semantics.prediction_subject,
        positive_label: semantics.positive_label,
        negative_label: semantics.negative_label,
        positive_display_name: semantics.positive_display_name,
        negative_display_name: semantics.negative_display_name,
      },
      null,
      2,
    ),
...
function buildPredictionSection(packageItem: EvidencePackage): string {
  ...
  if (semantics) {
    payload.predicted_label_display =
      prediction.predicted_label === semantics.positive_label
        ? semantics.positive_display_name
        : prediction.predicted_label === semantics.negative_label
...
function buildPolicySection(packageItem: EvidencePackage): string {
  ...
  if (targetSemantics) {
    ...
    const anchor = prediction.predicted_label === targetSemantics.positive_label
      ? targetSemantics.positive_direction_phrase
      : prediction.predicted_label === targetSemantics.negative_label
        ? targetSemantics.negative_direction_phrase
        : null;
    rules.push(
      `Khi mô tả nhãn dự đoán, phải liên hệ rõ với endpoint: ${targetSemantics.prediction_subject}.`,
    );
    if (anchor) {
      rules.push(
        `prediction_summary phải giữ cụm định hướng "${anchor}" để hướng rủi ro có thể được kiểm tra deterministic.`,
      );
    }
  }
...
function resolveTargetSemantics(
  packageItem: EvidencePackage,
): TargetSemanticsPayload | null {
  const candidate = packageItem.prompt_payload.target_semantics
    ?? packageItem.target_semantics
    ?? null;
  if (!candidate || !hasCompleteTargetSemantics(candidate)) return null;
  return candidate;
}

function hasCompleteTargetSemantics(
  value: unknown,
