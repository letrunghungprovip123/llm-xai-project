...
async function generateOne(
  ...
  const datasetContext = buildDatasetContext(packageItem);
  const targetSemantics = packageItem.prompt_payload.target_semantics
  ...
  const record: GenerationRecord = {
    ...
    ...(targetSemantics ? { target_semantics: targetSemantics } : {}),
...
function buildCaseMetadata(packageItem: EvidencePackage): CaseMetadata {
  ...
  const legacyCustomer = (isObject(internal.customer) ? internal.customer : {}) as Record<string, unknown>;
  const internalCase = (isObject(internal.case) ? internal.case : {}) as Record<string, unknown>;
  const topCase = (isObject(packageItem.case) ? packageItem.case : {}) as Record<string, unknown>;
  const caseSource = Object.keys(topCase).length > 0
    ? topCase
    : Object.keys(internalCase).length > 0
      ? internalCase
      : legacyCustomer;
  const groundTruth = (isObject(internal.ground_truth) ? internal.ground_truth : {}) as Record<string, unknown>;

  const sourceEntityId = readStringOrNumber(
    caseSource.source_entity_id
      ?? caseSource.case_id
      ?? legacyCustomer.SK_ID_CURR
      ?? legacyCustomer.customer_id,
  );
  const legacyCustomerId = readStringOrNumber(
    legacyCustomer.SK_ID_CURR
      ?? legacyCustomer.customer_id
      ?? sourceEntityId,
  );
  const selectionStratum =
    typeof caseSource.case_type === "string" ? caseSource.case_type : null;
  ...
  const datasetContext = buildDatasetContext(packageItem);
  if (!datasetContext) return base;
  return {
    ...base,
    dataset_id: datasetContext.dataset_id,
    ...(datasetContext.experiment_id
      ? { experiment_id: datasetContext.experiment_id }
      : {}),
    case_id: datasetContext.case_id,
    source_entity_id: datasetContext.source_entity_id,
    ...(datasetContext.entity_type
      ? { entity_type: datasetContext.entity_type }
      : {}),
  };
}
...
function buildDatasetContext(
  packageItem: EvidencePackage,
): DatasetContextSnapshot | null {
  const internal = packageItem.internal_metadata || {};
  const internalCase = (isObject(internal.case) ? internal.case : {}) as Record<string, unknown>;
  const topCase = (isObject(packageItem.case) ? packageItem.case : {}) as Record<string, unknown>;
  const caseSource = Object.keys(topCase).length > 0 ? topCase : internalCase;
  const datasetId = packageItem.dataset?.dataset_id
    ?? (typeof internal.dataset_id === "string" ? internal.dataset_id : null);
  const experimentId = packageItem.experiment?.experiment_id
    ?? (typeof internal.experiment_id === "string" ? internal.experiment_id : null);
  const caseId = typeof caseSource.case_id === "string"
    ? caseSource.case_id
    : typeof caseSource.case_id === "number"
      ? String(caseSource.case_id)
      : null;
  if (!datasetId || !caseId) return null;
  const sourceEntityId = readStringOrNumber(caseSource.source_entity_id);
  const entityType = typeof caseSource.entity_type === "string"
    ? caseSource.entity_type
    : null;
  return {
