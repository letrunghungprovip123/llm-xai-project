...
## Claims V3 semantic migration

`llm:claims-finalize-v3` creates a new lineage from the historical finalized
claims without overwriting it. The migration:

1. reads the source-controlled 260-row semantic-correction fixture;
2. drops reviewed incomplete fragments;
3. retypes or splits the 14 reviewed factor-level propositions;
4. repairs four explicit low-risk directions;
5. preserves three reviewed experimental contradictions;
6. assigns semantic subtypes to every surviving claim;
7. enforces the frozen claim-type compatibility matrix;
8. reconstructs every source span against the canonical generation document.

Every output record uses `claims_v3` and carries `parent_claim_id`,
`proposition_status`, `finalizer_version`, and
`finalization_policy_version`. The stage makes no provider calls.
