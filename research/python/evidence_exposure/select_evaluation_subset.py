...
def read_case_metadata(record: dict[str, Any]) -> dict[str, Any]:
    direct = record.get("case")
    if isinstance(direct, dict):
        return direct
    internal = read_internal_metadata(record)
    value = internal.get("case")
    if isinstance(value, dict):
        return value
    return {}
...
def get_stratum(record: dict[str, Any]) -> str | None:
    case = read_case_metadata(record)
...
def get_case_id(record: dict[str, Any]) -> str:
    # Multi-dataset packages expose the stable canonical case identity directly.
    # Historical v1 packages do not, so retain source_ir_id as an exact fallback.
    case = read_case_metadata(record)
    ...
    if value is None:
        raise ValueError("Record is missing both case.case_id and source_ir_id.")
    return str(value)
