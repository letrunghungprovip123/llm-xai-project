...
from ..common.dataframes import count_infinite_values as count_inf
from ..common.paths import DEFAULT_PATHS


PROJECT_ROOT = DEFAULT_PATHS.project_root

SPLIT_DIR = DEFAULT_PATHS.processed_dir / "splits"

TREE_DIR = DEFAULT_PATHS.processed_dir / "model_ready" / "tree"
LINEAR_DIR = DEFAULT_PATHS.processed_dir / "model_ready" / "linear"
...
def create_simple_imputer(strategy: str) -> Any:
    """Khởi tạo imputer tại runtime để validation core không phụ thuộc sklearn."""
...
def create_standard_scaler() -> Any:
    """Khởi tạo scaler dùng riêng cho linear pipeline."""
...
def create_one_hot_encoder() -> Any:
...
def transform_numeric(
    ...
    imputer = create_simple_imputer(strategy="median")
...
def transform_binary(
    ...
    imputer = create_simple_imputer(strategy="most_frequent")
...
def save_artifacts_and_registries(
    ...
    # joblib chỉ cần ở writer boundary, sau khi toàn bộ validation đã pass.
    import joblib
