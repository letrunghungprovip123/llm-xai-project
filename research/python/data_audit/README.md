...
Chạy bằng `npm run research -- ml:data-audit`. Output mặc định nằm trong `data/reports/` và `data/manifests/`. Thiếu file, duplicate key hoặc quan hệ không hợp lệ làm stage fail. Unit/regression liên quan nằm trong `tests/research/python/`.
