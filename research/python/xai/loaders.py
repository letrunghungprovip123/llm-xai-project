...
def load_model_bundle(
    path: Path,
    *,
    enforce_legacy_expectations: bool = True,
) -> XAIModelBundle:
    """Load one selected model bundle.

    Legacy Batch G keeps strict HGB/tree expectations by default. New
    multi-dataset callers can explicitly disable only that historical winner
    assertion while retaining bundle/schema/predict_proba validation.
    """
    import joblib
    ...
    if enforce_legacy_expectations and model_name != EXPECTED_BEST_MODEL_NAME:
        raise ValueError(
            "Loaded model is not the expected legacy best model. "
            f"Expected {EXPECTED_BEST_MODEL_NAME!r}, got {model_name!r}."
        )

    if enforce_legacy_expectations and dataset_branch != EXPECTED_DATASET_BRANCH:
        raise ValueError(
            "Loaded model dataset branch is not the legacy expected branch. "
