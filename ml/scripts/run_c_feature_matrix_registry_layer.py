...
def main() -> None:
    ...
    final_matrix_path = PROCESSED_DIR / "feature_matrix_full.parquet"
    target_path = PROCESSED_DIR / "target_full.parquet"

    print("[Batch C] Saving feature_matrix_full.parquet ...")
    matrix.to_parquet(final_matrix_path, index=False)

    print("[Batch C] Saving target_full.parquet ...")
    target.to_parquet(target_path, index=False)

    print("[Batch C] Building concept registry ...")
    concept_registry_path = REGISTRY_DIR / "concept_registry.yaml"
    save_yaml(CONCEPT_REGISTRY, concept_registry_path)

    print("[Batch C] Building feature registry ...")
    feature_registry = build_feature_registry(matrix)

    feature_registry_yaml_path = REGISTRY_DIR / "feature_registry.yaml"
    save_yaml(feature_registry, feature_registry_yaml_path)

    feature_registry_csv_path = REGISTRY_DIR / "feature_registry.csv"
    registry_df = registry_to_dataframe(feature_registry)
    registry_df.to_csv(feature_registry_csv_path, index=False)

    print("[Batch C] Validating final matrix ...")
    final_matrix_stats = validate_final_matrix(matrix, expected_rows=expected_rows)
    ...
    print("- data/processed/feature_matrix_full.parquet")
    print("- data/processed/target_full.parquet")
    print("- ml/registry/concept_registry.yaml")
    print("- ml/registry/feature_registry.yaml")
    print("- ml/registry/feature_registry.csv")
    print("- data/reports/feature_matrix_registry_report.md")
    print("- data/reports/batch_c_feature_matrix_registry_summary.md")
    print("- data/manifests/batch_c_feature_matrix_registry_summary.json")
