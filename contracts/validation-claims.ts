...
export const CLAIM_SUBTYPES_BY_TYPE = {
  prediction: [
    "OVERALL_LABEL",
    "OVERALL_PROBABILITY",
  ...
  feature_presence: ["FEATURE_MENTION"],
  feature_direction: ["FEATURE_RISK_DIRECTION"],
  concept_presence: ["CONCEPT_MENTION"],
  concept_direction: ["CONCEPT_RISK_DIRECTION"],
  magnitude: [
    "FEATURE_STRENGTH",
    "CONCEPT_STRENGTH",
  ...
  ranking: ["FEATURE_RANK", "CONCEPT_RANK"],
  numeric: [
    "PREDICTION_SCORE",
    "DECISION_THRESHOLD",
    "FEATURE_VALUE",
  ...
  recommendation: [
    ...
    "CAUTION_IN_DECISION_USE",
    "PRESCRIPTIVE_FINANCIAL_ACTION",
...
} as const satisfies Record<ClaimType, readonly string[]>;

export type ClaimSubtypeByType = {
  [Type in ClaimType]: (typeof CLAIM_SUBTYPES_BY_TYPE)[Type][number];
};

export type ClaimSubtype = ClaimSubtypeByType[ClaimType];
export type ClaimPropositionStatus = "COMPLETE" | "INCOMPLETE_FRAGMENT";
...
type AtomicClaimRecordV3Base = Omit<
  AtomicClaimRecord,
  "claim_schema_version" | "claim_type"
> & {
  claim_schema_version: "claims_v3";
  parent_claim_id: string | null;
  proposition_status: ClaimPropositionStatus;
  source_start: number;
  source_end: number;
...
// Discriminated union: a subtype cannot be paired with the wrong claim type.
export type AtomicClaimRecordV3 = {
  [Type in ClaimType]: AtomicClaimRecordV3Base & {
    claim_type: Type;
    claim_subtype: ClaimSubtypeByType[Type];
  };
}[ClaimType];
